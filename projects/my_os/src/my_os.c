...
/** valor de retorno de excepci��n a cargar en el LR */
#define EXC_RETURN 0xFFFFFFF9

/** id de tarea inv��lida */
//#define INVALID_TASK ((uint32_t)-1)
#define INVALID_TASK  TASK_COUNT
...
/* si una tarea ejecuta return, vengo ac�� */
...
static void task_create(
		...
		uint32_t stack_frame_size, /* el tama��o expresado en bytes */
...
/* rutina de selecci��n de pr��ximo contexto a ejecutarse
 * ac�� definimos la pol��tica de scheduling de nuestro os
 */
int32_t getNextContext(int32_t current_context)
{
	...
	if(previous_task == current_task && task_control_list[current_task].state != TASK_STATE_RUNNING){ // cambio por redy por running
		if (idle_task_control.state == TASK_STATE_RUNNING){
			idle_task_control.sp = current_context;
		}else{
			idle_task_control.state = TASK_STATE_RUNNING; // Init case all task are waiting
	...
	if (task_control_list[current_task].state == TASK_STATE_READY){
		if(idle_task_control.state == TASK_STATE_RUNNING){ // Change contex, before idle state
		...
		task_control_list[current_task].sp = current_context; // In this case continue de same task running
	}
